...
export async function uploadDocumentsAction(workspaceId: string, fileId: string, formData: FormData): Promise<ActionState<{ count: number; documents: Array<{ id: string; filename: string; duplicate: boolean }> }>> {
  ...
  try {
    ...
    for (const file of files) {
      ...
      const result = await createDocumentFromBuffer({ workspaceId, fileId, templateId: template.id, source: "upload", filename: file.name, mimeType: file.type, buffer, pageRange: pageRangeRaw || null, uploadBatchId })
      if (!result.duplicate) count++
      documents.push({ id: result.document.id, filename: result.document.filename, duplicate: result.duplicate })
...
export async function getDocumentProcessingStatusAction(workspaceId: string, documentIds: string[]): Promise<ActionState<Array<{ id: string; status: string; errorCode: string | null; filename: string }>>> {
  const user = await getCurrentUser()
  if (!(await requireMember(workspaceId, user.id))) return { success: false, error: NO_ACCESS }
  try {
