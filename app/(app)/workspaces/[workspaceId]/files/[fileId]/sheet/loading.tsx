...
export default function SheetLoading() {
  return <div className="flex min-h-0 flex-1 flex-col" aria-busy="true" aria-label="Loading spreadsheet">
    <div className="flex items-center gap-2 border-b px-3 py-2">
      ...
      <div className="ml-auto h-7 w-20 animate-pulse rounded-md bg-stone-200" />
    </div>
    <div className="flex items-center gap-2 border-b px-3 py-2">
      {Array.from({ length: 8 }).map((_, index) => <div key={index} className="h-6 w-6 animate-pulse rounded bg-stone-100" />)}
      <div className="h-6 w-24 animate-pulse rounded bg-stone-100" />
    </div>
    <div className="min-h-0 flex-1 bg-white">
      <div className="h-7 animate-pulse border-b bg-stone-100" />
      {Array.from({ length: 12 }).map((_, index) => <div key={index} className="flex h-7 border-b">
        <div className="w-10 shrink-0 animate-pulse bg-stone-50" />
