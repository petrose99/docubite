  ...
  return <main className="mx-auto w-full max-w-5xl space-y-5 px-4 py-6 md:px-6">
    <header>
      <div className="flex items-center gap-2">
        <h1 className="text-xl font-bold text-slate-900">Docu Library</h1>
        <SectionIntro section="library" workspaceId={workspaceId} />
      </div>
    ...
    <div className="flex items-center gap-4">
      <nav className="flex items-center gap-1 text-sm">
        {STAGE_TABS.map((tab) => (
          <Link
            key={tab.key}
            href={`${base}/library${tab.key === "ready" ? "" : `?stage=${tab.key}`}`}
