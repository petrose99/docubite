...
export function PeriodFilter({ workspaceId, current, from, to }: { workspaceId: string; current: string; from: string; to: string }) {
  ...
  return <div className="flex flex-wrap items-center gap-3">
    <nav className="flex gap-1 rounded-md border bg-slate-50 p-0.5">
      {PILLS.map((pill) => {
        const active = current === pill.value
        return <Link key={pill.value} href={pill.value === "12m" ? base : `${base}?period=${pill.value}`}
          className={`rounded px-3 py-1 text-sm font-medium ${active ? "bg-white text-emerald-800 shadow-sm" : "text-slate-500 hover:text-slate-800"}`}>
          {pill.label}
        </Link>
      })}
    </nav>
    <form className="flex items-center gap-1.5 text-sm" action={base} method="get">
      ...
      <button type="submit" className="rounded-md border bg-white px-2.5 py-1 font-medium text-slate-700 hover:bg-slate-50">Apply</button>
    </form>
  </div>
}
