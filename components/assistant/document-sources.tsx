  ...
  return (
    <div className="rounded-lg border bg-white">
      <button
        ...
        className="flex w-full items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-stone-700">
        <FileSearch className="h-3.5 w-3.5 shrink-0 text-emerald-700" />
        Sources
        <span className="rounded-full bg-stone-100 px-1.5 text-[11px] text-stone-500">{hits.length}</span>
        <ChevronDown className={`ml-auto h-3.5 w-3.5 shrink-0 text-stone-400 transition-transform ${open ? "" : "-rotate-90"}`} />
      </button>
      {open && (
          ...
          {hits.map((hit, index) => (
            <li key={index}>
              <button
                ...
                className="flex w-full flex-col gap-0.5 px-2.5 py-1.5 text-left hover:bg-stone-50">
                <span className="flex w-full items-center gap-1.5">
                  <FileText className="h-3 w-3 shrink-0 text-stone-400" />
                  <span className="min-w-0 flex-1 truncate text-xs text-stone-700">{middleTruncate(hit.filename)}</span>
                  {hit.page != null && (
                    <span className="shrink-0 rounded bg-emerald-50 px-1 font-mono text-[11px] text-emerald-800">p.{hit.page}</span>
                  )}
                </span>
                {hit.snippet && <span className="line-clamp-2 text-[11px] text-stone-500">{hit.snippet}</span>}
              </button>
            </li>
          ))}
        </ul>
