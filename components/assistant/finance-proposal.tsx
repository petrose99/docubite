...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  no_active_connection: "No active accounting connection â€” connect one from Settings â†’ Integrations first.",
 ...
 * â€” the confirm-before-execute counterpart to the sheet assistant's write-then-undo cell tools
...
export function FinanceProposalPart({ workspaceId, state, output }: {
  ...
  if (state !== "output-available" || !output) {
    return <p className="flex items-center gap-1.5 text-xs text-slate-500"><Loader2 className="h-3 w-3 animate-spin" />Preparingâ€¦</p>
  }
  if ("error" in output) {
  ...
  if (resolved === "accepted") {
    return <p className="flex items-center gap-1.5 rounded-md bg-emerald-50 px-2.5 py-2 text-xs text-emerald-800"><CircleCheck className="h-3.5 w-3.5 shrink-0" />{output.summary} â€” done</p>
  }
