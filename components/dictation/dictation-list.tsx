...
function StatusBadge({ row }: { row: DictationRow }) {
  ...
  if (row.status === "queued" || row.status === "processing") {
    return <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600"><Loader2 className="h-3 w-3 animate-spin" />Transcribing</span>
  }
  if (row.draft) {
    return <span className="inline-flex items-center gap-1 rounded-full bg-indigo-50 px-2 py-0.5 text-xs font-medium text-indigo-800"><FileText className="h-3 w-3" />Draft v{row.draft.version} — unsigned</span>
  }
  if (row.status === "needs_review") {
    return <span className="inline-flex items-center gap-1 rounded-full bg-indigo-50 px-2 py-0.5 text-xs font-medium text-indigo-800"><TriangleAlert className="h-3 w-3" />Fields missing</span>
  }
  return <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600">Awaiting review</span>
}
...
export function DictationList({ workspaceId, dictations }: { workspaceId: string; dictations: DictationRow[] }) {
  ...
  if (!rows.length) {
    return (
      <section className="rounded-xl border border-dashed border-slate-300 bg-slate-50/50 px-6 py-12 text-center">
        <Mic className="mx-auto h-6 w-6 text-slate-400" />
        <p className="mt-2 text-sm font-medium text-slate-700">No dictations yet</p>
        <p className="mt-1 text-xs text-slate-500">Record one above and it will appear here to verify and sign.</p>
      </section>
    )
  }
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      <h2 className="border-b border-slate-100 px-4 py-2.5 text-sm font-semibold text-slate-900">Cases</h2>
          ...
          <li key={row.id}>
            <Link
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium text-slate-900">{row.summary ?? row.filename}</p>
                <p className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-slate-500">
                  <span>{row.templateName}</span>
                  <span aria-hidden>·</span>
                  <span>{new Date(row.receivedAt).toLocaleString()}</span>
                  {row.transcriptEdited && <span className="inline-flex items-center gap-1 text-slate-500"><PencilLine className="h-3 w-3" />transcript edited</span>}
                </p>
              </div>
              <StatusBadge row={row} />
