...
function StatusBadge({ staged }: { staged: StagedFile }) {
  ...
  const searchable = settled && staged.searchable
    ? <span className="inline-flex items-center gap-1 text-xs font-medium text-slate-400" title="Indexed for document search — ask the AI Assistant about its contents"><FileSearch className="h-3 w-3" />Searchable</span>
    : settled && staged.indexing
      ? <span className="inline-flex items-center gap-1 text-xs font-medium text-slate-400" title="Being indexed for document search"><Loader2 className="h-3 w-3 animate-spin" />Indexing…</span>
      : null

  const flagged = staged.flaggedFields?.length
    ? <span className="inline-flex items-center gap-1 text-xs font-medium text-indigo-600" title={flaggedFieldsTitle(staged.flaggedFields)}>
        <AlertTriangle className="h-3 w-3" />{staged.flaggedFields.length} field{staged.flaggedFields.length === 1 ? "" : "s"}
      </span>
    : null

  let status: ReactNode = null
  if (staged.status === "uploading") status = <span className="inline-flex items-center gap-1 text-xs font-medium text-slate-500"><Loader2 className="h-3 w-3 animate-spin" />Uploading</span>
  ...
  else if (staged.status === "attention") status = <span className="inline-flex items-center gap-1 text-xs font-medium text-indigo-600" title={staged.flaggedFields?.length ? flaggedFieldsTitle(staged.flaggedFields) : undefined}><AlertTriangle className="h-3 w-3" />Review</span>
  ...
  else if (staged.status === "duplicate") status = <span className="inline-flex items-center gap-1 text-xs font-medium text-slate-500" title="This exact file was already extracted"><Copy className="h-3 w-3" />Already extracted</span>
...
export function FileRow({ staged, sourceUrl, busy, onExtract, onReprocess, onReextractAdaptively, onRemove, onDiff }: {
  ...
  return <div className="flex items-center gap-2 rounded-md border border-transparent px-2 py-1.5 text-sm hover:border-slate-200 hover:bg-slate-50">
    <span className="min-w-0 flex-1 truncate font-medium text-slate-800" title={staged.relativePath ? `${staged.relativePath}/${staged.filename}` : staged.filename}>
      {staged.relativePath && <span className="text-slate-400">{staged.relativePath}/</span>}{staged.filename}
    </span>
    <span className="shrink-0 text-xs text-slate-400">{formatSize(staged.sizeBytes)}</span>
    <StatusBadge staged={staged} />
    <div className="flex shrink-0 items-center gap-0.5 text-slate-500">
      {previewSrc && <button type="button" className="rounded p-1 hover:bg-slate-200 hover:text-slate-800" title="Preview" onClick={() => setPreview(true)}><Eye className="h-4 w-4" /></button>}
      {previewSrc && <a className="rounded p-1 hover:bg-slate-200 hover:text-slate-800" title="Download" href={previewSrc} download={staged.filename}><Download className="h-4 w-4" /></a>}
      {staged.status === "staged" && <button type="button" className="ml-1 rounded-md border border-slate-300 px-2 py-0.5 text-xs font-medium hover:bg-white disabled:opacity-50" disabled={busy} onClick={onExtract} title="Extract this file now"><span className="inline-flex items-center gap-1"><Play className="h-3 w-3" />Extract</span></button>}
      {onDiff && (staged.status === "done" || staged.status === "attention") && staged.documentId && <button type="button" className="rounded p-1 hover:bg-slate-200 hover:text-slate-800" title="Diff vs the last document of this shape" onClick={onDiff}><GitCompare className="h-4 w-4" /></button>}
      {(staged.status === "done" || staged.status === "attention" || staged.status === "failed" || staged.status === "duplicate") && staged.documentId && <button type="button" className="rounded p-1 hover:bg-slate-200 hover:text-slate-800 disabled:opacity-50" disabled={busy} title="Re-process" onClick={onReprocess}><RotateCw className="h-4 w-4" /></button>}
      {(staged.status === "done" || staged.status === "attention") && staged.documentId && <button type="button" className="rounded p-1 hover:bg-slate-200 hover:text-slate-800 disabled:opacity-50" disabled={busy} title="Re-extract adaptively — discover this document's real line-item columns" onClick={onReextractAdaptively}><Sparkles className="h-4 w-4" /></button>}
    ...
    {preview && previewSrc && typeof document !== "undefined" && createPortal(<div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 p-6" onClick={() => setPreview(false)}>
      <div className="flex h-full max-h-[85vh] w-full max-w-3xl flex-col overflow-hidden rounded-xl bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}>
        <div className="flex items-center justify-between border-b px-4 py-2.5">
          <span className="truncate text-sm font-semibold text-slate-800">{staged.filename}</span>
          <button type="button" className="rounded p-1 text-slate-500 hover:bg-slate-100" onClick={() => setPreview(false)}><X className="h-4 w-4" /></button>
        </div>
