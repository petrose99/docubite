...
export default function ProvenancePdf({ href, target }: { href: string; target?: ProvenanceTarget | null }) {
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col bg-stone-100">
      ...
      <div className="flex items-center gap-1 border-b bg-white/80 px-2 py-1">
        <button type="button" onClick={() => setZoom((z) => Math.max(0.4, z - 0.2))} className="rounded p-1 text-stone-500 hover:bg-stone-100 hover:text-stone-800" title="Zoom out"><Minus className="h-4 w-4" /></button>
        <span className="w-12 text-center text-xs tabular-nums text-stone-500">{Math.round(zoom * 100)}%</span>
        <button type="button" onClick={() => setZoom((z) => Math.min(3, z + 0.2))} className="rounded p-1 text-stone-500 hover:bg-stone-100 hover:text-stone-800" title="Zoom in"><Plus className="h-4 w-4" /></button>
        <button type="button" onClick={fitWidth} className="rounded p-1 text-stone-500 hover:bg-stone-100 hover:text-stone-800" title="Fit width"><Maximize2 className="h-4 w-4" /></button>
      </div>
      ...
      <div ref={hostRef} className={`min-h-0 flex-1 overflow-auto p-4 ${status === "ready" ? "" : "hidden"}`} />
    </div>
  )
}
