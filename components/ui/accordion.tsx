...
export function Accordion({ children, className }: { children: React.ReactNode; className?: string }) {
  return <div className={cn("divide-y divide-slate-200 border-y border-slate-200", className)}>{children}</div>
}
...
export function AccordionItem({ question, children, defaultOpen = false }: { question: string; children: React.ReactNode; defaultOpen?: boolean }) {
  return (
    <details className="group" open={defaultOpen}>
      <summary className="flex cursor-pointer list-none items-center justify-between gap-6 py-5 text-left font-display text-lg font-semibold tracking-[-0.01em] text-slate-900 hover:text-emerald-800 [&::-webkit-details-marker]:hidden">
        {question}
      ...
      <div className="max-w-3xl pb-5 leading-7 text-slate-600">{children}</div>
    </details>
  )
}
