...
export type AsrGateWorkspace = { productMode: string; hipaaMode: boolean; asrExternalAllowed: boolean }
 ...
 * caller of it asks. That is not enough for a clinical, hipaaMode workspace: Whisper-via-HF and
 ...
 * non-clinical or non-hipaaMode workspace has made no such presumption, so it is unaffected —
 * this only narrows the clinical+hipaaMode case, never widens any other one. */
export function isAsrAllowed(workspace: AsrGateWorkspace): boolean {
  if (!config.asr.enabled) return false
  if (workspace.productMode !== "clinical") return true
