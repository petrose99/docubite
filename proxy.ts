 ...
 * refreshed, and to bounce a signed-out visitor away from the two prefixes that require one.
 *
 ...
 * carry a stale token into the first protected navigation, and Supabase's own refresh only
 * happens where this middleware runs. The cost is a JWT verification (local, via JWKS — no round
 * trip to Supabase for a project using the default asymmetric signing keys) on every matched
 * request; that's the trade Supabase's own docs recommend this exact matcher shape for.
 *
